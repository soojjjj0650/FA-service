...
def section_title(slide, l, t, w, text):
    ...
    txt(slide, text, l + Cm(0.3), t, w, Cm(0.62),
...
def flow_node(slide, l, t, w, h, top_text, bot_text=""):
    ...
    if bot_text:
        txt(slide, top_text, l, t + Cm(0.1), w, Cm(0.45),
            sz=9, bold=True, color=NAVY, align=PP_ALIGN.CENTER)
        txt(slide, bot_text, l, t + Cm(0.52), w, Cm(0.38),
            sz=7.5, color=MGRAY, align=PP_ALIGN.CENTER)
    else:
        txt(slide, top_text, l, t + Cm(0.15), w, h,
            sz=9, bold=True, color=NAVY, align=PP_ALIGN.CENTER)

def arrow_r(slide, l, t, w=Cm(0.65), h=Cm(0.38)):
...
paras = [
    ...
    "통신사 이관 이슈 조기 분류 → 진성 단말 이슈에 집중 가능",
]
for p in paras:
...
cy += Cm(0.65)
probs = [
    "주말·야간 대응 불가  (본사 인력 부재)",
    "수동 조회 대기시간 발생",
    "반복 업무로 인력 비효율",
]
for p in probs:
    txt(slide, f"• {p}", C1L + Cm(0.15), cy, CW - Cm(0.15), Cm(0.45), sz=9)
    cy += Cm(0.46)
...
for g in goals:
    txt(slide, f"▸  {g}", C2L + Cm(0.15), cy, CW - Cm(0.15), Cm(0.48), sz=9)
    cy += Cm(0.49)

# 컬럼3: 주요 기능 & 기대 효과
cy = TOP_T
section_title(slide, C3L, cy, CW, "주요 기능")
cy += Cm(0.65)
feats = [
    ("SN 기반 자동 분석",    "Knox Messenger · Adaptive Card · 다중 SN"),
...
for title, desc in feats:
    txt(slide, title, C3L + Cm(0.15), cy, CW - Cm(0.15), Cm(0.4),
        sz=9, bold=True, color=DGRAY)
    txt(slide, desc,  C3L + Cm(0.15), cy + Cm(0.38), CW - Cm(0.15), Cm(0.35),
        sz=8, color=MGRAY, italic=True)
    cy += Cm(0.82)

# ── 상·하단 구분선 ────────────────────────────────────────────────────────────
DIV_Y = TOP_T + TOP_H + Cm(0.18)
divider_h(slide, PAD, DIV_Y, W - PAD * 2)

# 시스템 구성 섹션 제목
FLOW_TITLE_Y = DIV_Y + Cm(0.1)
section_title(slide, PAD, FLOW_TITLE_Y, W - PAD * 2, "시스템 구성")

# ── 하단 흐름도 (좌→우) ──────────────────────────────────────────────────────
FLOW_T = FLOW_TITLE_Y + Cm(0.68)
FLOW_H_AVAIL = H - FLOW_T - Cm(0.65)   # 푸터 제외
NW = Cm(0.02)   # 노드 간격

nodes = [
    ("👤 FA 담당자",          "SN 입력"),
    ("💬 Knox Messenger",      "메시지 전달"),
    ("⚙ FA 분석 자동화 서버", "분석 요청 수신"),
    ("🗄 빅데이터 포털",       "자동 조회"),
    ("📄 분석 결과 생성",      "PDF / HTML"),
...
N = len(nodes)
ARR_W = Cm(0.7)
NODE_W = (W - PAD * 2 - ARR_W * (N - 1)) / N
NODE_H = min(Cm(1.1), FLOW_H_AVAIL)
NODE_T = FLOW_T + (FLOW_H_AVAIL - NODE_H) / 2
...
for i, (top, bot) in enumerate(nodes):
    ...
    if i < N - 1:
        al = nl + NODE_W
        arrow_r(slide, al, NODE_T + NODE_H / 2 - Cm(0.19), ARR_W, Cm(0.38))
