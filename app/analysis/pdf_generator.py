...
async def html_to_pdf(html_path: str, pdf_path: str) -> bool:
    ...
    try:
        async with async_playwright() as p:
            ...
            # NETA 조회 시작될 때까지 대기 (최대 15초)
            try:
                await page.wait_for_function(
                    "() => document.body.innerText.includes('NETA 조회중')",
                    timeout=15000,
                )
                logger.info("[PDF] NETA 조회 시작 감지 → 완료 대기 중...")
                # NETA 조회 완료될 때까지 대기 (최대 120초)
                await page.wait_for_function(
                    "() => !document.body.innerText.includes('NETA 조회중')",
                    timeout=120000,
                )
                logger.info("[PDF] NETA 조회 완료")
            except Exception:
                logger.warning("[PDF] NETA 조회 감지 안됨 — 그대로 진행")
