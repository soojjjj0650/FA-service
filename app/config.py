...
class Settings(BaseSettings):
    ...
    # ─── CSV 다운로드 경로 ────────────────────────────────────────────────────
    CSV_DOWNLOAD_PATH: str = r"D:\FA_Service\userdata"
    ...
    MAIL_SCHEDULE_HOUR: int = 17            # 자동 실행 시각 (24h)
    MAIL_SCHEDULE_MINUTE: int = 30          # 자동 실행 분 (기본 17:30)
    ...
    # Knox 카드 콜백 URL (Knox에 등록된 수신 URL 그대로, 비워두면 BASE_URL:PORT/message 사용)
    KNOX_SERVER_URL: str = ""
    KNOX_RECEIVE_URL: str = ""

    # ─── 쿼리 조회 기간 ──────────────────────────────────────────────────────
    QUERY_LOOKBACK_DAYS: int = 14       # SQL INTERVAL N DAY (데이터 조회 기간)
    BATCH_SN_LIMIT: int = 0             # 0 = 전체 실행
